...
export function ServiceCard({ icon, title, body, href, linkLabel, tabIndex }: ServiceCardProps) {
  return (
    <Card as="article" interactive className="flex h-full w-full flex-col bg-white/95">
      <IconBadge name={icon} />
      <h3 className="mt-5 text-[1.125rem] font-semibold text-navy-900">
        <Link href={href} tabIndex={tabIndex} className="after:absolute after:inset-0 after:rounded-xl focus-visible:outline-none">
          {title}
        </Link>
      </h3>
...
export function ServicesGrid({ content }: { content: HomeContent["services"] }) {
  ...
  const move = (direction: 1 | -1, announce = true) => {
    setPage((current) => {
      ...
      if (announce) setAnnouncement(`Showing service set ${next + 1} of ${pageCount}.`);
      return next;
    });
  ...
  return (
    <Section id="services" tone="surface" aria-labelledby="services-title" className="overflow-hidden">
      ...
      >
        ...
        <div className="-mx-2.5 overflow-hidden px-0.5 py-2">
          <ul
            className={cn("flex items-stretch", reducedMotion ? "transition-none" : "transition-transform duration-500 ease-[cubic-bezier(.22,1,.36,1)]")}
            style={{
        ...
        <div className="mt-7 flex flex-wrap items-center justify-center gap-4 sm:justify-between">
          <div className="flex items-center gap-2">
            ...
            >
              <Icon name="arrow-right" className="size-4 rotate-180" strokeWidth={2.4} />
            </button>
            <button
              type="button"
              onClick={() => move(1)}
          ...
          <div className="flex items-center gap-2" aria-label={`Service set ${currentPage + 1} of ${pageCount}`}>
            {Array.from({ length: pageCount }, (_, index) => (
              <button
                ...
                onClick={() => {
                  setPage(index);
                  setTimerVersion((version) => version + 1);
                  setAnnouncement(`Showing service set ${index + 1} of ${pageCount}.`);
                }}
                className={cn(
                  "h-2.5 rounded-full transition-[width,background-color] duration-300",
                  currentPage === index ? "w-7 bg-brand-600" : "w-2.5 bg-line-strong hover:bg-brand-300",
                )}
